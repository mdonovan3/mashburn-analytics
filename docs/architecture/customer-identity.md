...
incomplete.** :white_check_mark: **Fixed** — `customer_email` added to
`SWYM_WISHLIST_EVENTS` in `ingestion/schemas.py`, generated by both mock
data scripts (`generate_seed.py`, `generate_daily.py`) using the same
customer records as the rest of the mock data, and documented in
`sources.yml`. Still needs the same field added to the real dlt source
once that's built — tracked in [Status & Roadmap](../status.md).
